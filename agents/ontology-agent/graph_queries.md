...
> **API Limitation**: Graph Query Sets can be created via API, but **queries must be added manually in the Fabric UI portal**.

---

## Graph Model

The Graph Model is **auto-generated** from the Ontology:
- Each entity type becomes a **node type**
- Each relationship type becomes an **edge type**
- Properties from entity types become node properties
- Timeseries properties are available through the graph

### Generating / Refreshing the Graph Model

**Option 1 — Fabric UI**: Open the Ontology → click "Refresh graph model"

**Option 2 — From Semantic Model**: Fabric UI → open Semantic Model → "Generate Ontology" → creates entity types + initial relationships → Graph Model auto-generates

**Option 3 — API**: Create the Ontology with `updateDefinition` → the Graph Model regenerates automatically when bindings are valid.
...
| `OPTIONAL MATCH` | Left-join style match | Returns `null` if no match |
...
#### 11. Full Equipment Hierarchy (3-hop: Refinery → ProcessUnit → Equipment)
```gql
...
#### 19. Employee Workload (Maintenance + Shifts + Quality)
```gql
MATCH (e:Employee)
...
| API creation | Create item ✅, push queries ❌ (UI only) | Kusto REST mgmt API ✅ |
| Item type | GraphQuerySet | N/A (queries run in dashboard/agent) |
| Joins | Implicit via graph edges | Explicit `join` operator |
| Multi-hop | Native (chain `->` patterns) | Requires nested joins |

**When to use which**:
- **GQL**: "Show me all equipment under Refinery X and their sensors" (graph traversal)
- **KQL**: "Show me sensor readings in the last 24 hours where value > threshold" (time-series)
...
Invoke-WebRequest -Uri "$apiBase/workspaces/$WorkspaceId/items" `
    -Method POST -Headers $headers -Body $gqsBody
```

> **Then open in Fabric UI to add GQL queries.** There is no API to push query content into a Graph Query Set.

---

## Graph Schema — Oil & Gas Refinery
